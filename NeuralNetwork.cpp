...
void NeuralNetwork::propagateForward(RowVector<float> &input)
{
  ...
  for (uint i = 1; i < topology.size(); i++)
  {
    ...
    {
      cacheLayers[i]->setValue(ii, neuronLayers[i]->coeffRef(ii));
      cout << i << " " << ii << " inner potential:" << neuronLayers[i]->coeffRef(ii) << endl;
      // cout << i << " " << ii << " activated:" << activationFunction(neuronLayers[i]->coeffRef(ii)) << endl;
      neuronLayers[i]->setValue(ii, activationFunction(neuronLayers[i]->coeffRef(ii)));
    }
    if (i < topology.size() -1)
      neuronLayers[i]->setValue(topology[i], 1);
    ...
    else
    {
      for (uint c = 0; c < weights[i]->cols(); c++)
      {
        for (uint r = 0; r < weights[i]->rows(); r++)
        {
          ...
          // cout << "deltas: " << i + 1 << " pos: " << c << " value " << deltas[i + 1]->coeffRef(c) << endl;
          // cout << "y: " << i + 1 << " pos: " << r << " value " << neuronLayers[i]->coeffRef(r) << endl;
          // cout << "layers len: " << i + 1 << " len " << neuronLayers[i]->length() << endl;
          // cout << "weight: " << i << " pos: " << r << ", " << c << " prev value " << weights[i]->operator[]({r, c}) << " set to " << num << endl;
          weights[i]->operator[]({r, c}) = num;
        }
      }
...
void NeuralNetwork::train(std::vector<RowVector<float> *> input_data, std::vector<RowVector<float> *> output_data)
{
  ...
  {
    std::cout << "Input to neural network is : " << input_data[i]->coeffRef(0) << ", " << input_data[i]->coeffRef(1) << std::endl;
